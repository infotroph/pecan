...
##'         c(plot_variance_decomposition(x), ncol = 4))
plot_variance_decomposition <- function(plot.inputs,
  ...
  pv.plot <- base.plot + ggplot2::ggtitle("Variance") +
    ...
    ggplot2::geom_pointrange(
      ggplot2::aes(
        x = .data$points,
        y = sqrt(.data$variances),
        ymin = 0,
        ymax = sqrt(.data$variances)),
      size = 1.25)
...
plot_sensitivity <- function(sa.sample, sa.spline, trait, y.range = c(0, 50), median.i = 4, 
  ...
  saplot <- saplot + ## plot spline function
    ggplot2::geom_line(
      data = data.frame(x = post.x, y = sa.spline(post.x)),
    ...
    ggplot2::geom_point(
      ggplot2::aes(.data$x, .data$y),
      data = data.frame(
        x = sa.sample[median.i],
        y = sa.spline(sa.sample[median.i])),
      size = dotsize * 1.3) +
    ggplot2::scale_y_continuous(
      limits = range(pretty(y.range)),
      breaks = pretty(y.range, n = 3)[1:3]) +
    ggplot2::theme_bw() +
  ...
  if (!is.null(prior.sa.sample) & !is.null(prior.sa.spline)) {
    prior.x <- seq(
      from = min(prior.sa.sample),
      to = max(prior.sa.sample),
    ...
    saplot <- saplot + ## plot spline
      ggplot2::geom_line(
        ...
        size = linesize,
        color = "grey") + ## plot points used to evaluate spline
      ggplot2::geom_point(
        ggplot2::aes(.data$x, .data$y),
        data = data.frame(
          x = prior.sa.sample,
          y = prior.sa.spline(prior.sa.sample)),
        size = dotsize,
        color = "grey") + ## indicate location of medians
      ggplot2::geom_point(
        ggplot2::aes(.data$x, .data$y),
        data = data.frame(
          x = prior.sa.sample[median.i],
  ...
  saplot <- saplot +
    ggplot2::scale_x_continuous(units, limits = range(x.breaks), breaks = x.breaks)
  # print(saplot)
  return(saplot)
} # plot_sensitivity
