...
#'
#' This function reads and checks the input data, ensuring that the required
#' date and carbon pool exist, and that the site coordinates are valid.
#'
#' It ensures that the specified date and carbon pool are present in the input
#' data. It also checks the validity of the site coordinates and aligns the
#' number of rows between site coordinates and carbon data.
#'
#' @author Sambhav Dixit
#'
#' @param data_path Character. File path for .rds containing ensemble data.
#' @param coords_path Character. File path for .csv file containing the site
#'   coordinates, with columns named "lon" and "lat".
#' @param date Date. If SDA site run, format is yyyy/mm/dd; if NEON, yyyy-mm-dd.
#'   Restricted to years within the file supplied to 'data_path'.
#' @param carbon_pool Character. Carbon pool of interest. Name must match the
#'   carbon pool name found within the file supplied to 'data_path'.
#'
#' @return A list containing The read .rds data , The cleaned site coordinates,
#'   and the preprocessed carbon data.
#'
...
#'
#' Generates k-fold indices for cross-validation, allowing for flexible output
#'  formats.
#'
#' This function creates k-fold indices for cross-validation.
#'  It can return either training or test set indices, and the output can be in
#   list or vector format.
#'
#' @author Sambhav Dixit
#'
#' @param y Vector. A vector of outcome data or indices.
#' @param k Numeric. The number of folds to create.
#' @param list Logical. If TRUE, returns a list of fold indices.
#'   If FALSE, returns a vector.
#' @param returnTrain Logical. If TRUE, returns indices for training sets.
#'   If FALSE, returns indices for test sets.
#'
#' @return A list of k elements (if list = TRUE), each containing indices for a
#'   fold, or a vector of indices (if list = FALSE).
#' @noRd
#'
...
#'
#' This function uses either Random Forest or Convolutional Neural Network model
#'  based on the model_type parameter.
#'
#' This function will downscale forecast data to unmodeled locations using
#'  covariates and site locations
#'
#' @author Joshua Ploshay, Sambhav Dixit
#'
#' @param preprocessed List. Preprocessed data returned as an output from the
#'  SDA_downscale_preprocess function.
#' @param date Date. If SDA site run, format is yyyy/mm/dd; if NEON, yyyy-mm-dd.
#'  Restricted to years within file supplied to 'preprocessed' from the 'data_path'.
#' @param carbon_pool Character. Carbon pool of interest. Name must match carbon
#'  pool name found within file supplied to 'preprocessed' from the 'data_path'.
#' @param covariates SpatRaster stack. Used as predictors in downscaling.
#'  Layers within stack should be named. Recommended that this stack be
#'  generated using 'covariates' instructions in assim.sequential/inst folder
#' @param model_type Character. Either "rf" for Random Forest or "cnn" for
#'  Convolutional Neural Network. Default is Random Forest.
#' @param seed Numeric or NULL. Optional seed for random number generation.
#'  Default is NULL.
#'
#' @return A list containing the training and testing data sets, models,
#'  predicted maps for each ensemble member, and predictions for testing data.
#'
...
#'
#' Takes the output from the SDA_downscale function and computes various
#'  performance metrics for each ensemble. It provides a way to evaluate the
#'  accuracy of the downscaling results without modifying the main downscaling
#'  function.
#'
#' This function calculates performance metrics for the downscaling results.
#'  It computes Mean Squared Error (MSE), Mean Absolute Error (MAE),
#'  and R-squared for each ensemble.
#' The function uses the actual values from the testing data and the predictions
#'  generated during the downscaling process.
#'
#' @author Sambhav Dixit
#'
#' @param downscale_output List. Output from the SDA_downscale function,
#'  containing data, models, maps, and predictions for each ensemble.
#' @param carbon_pool Character. Name of the carbon pool used in the downscaling
#'  process.
#'
#' @return A list of metrics for each ensemble, where each element contains MAE,
#'  MSE ,R_squared ,actual values from testing data and predicted values for the
#'  testing data
#'
