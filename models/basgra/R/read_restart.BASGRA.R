...
##' @export
##' 
read_restart.BASGRA <- function(outdir, runid, stop.time, settings, nc_var.names, params) {
  ...
  ens <- PEcAn.utils::read.output(runid = runid,
                                  ...
                                  variables = nc_var.names)
  
  last <- length(ens[[1]])
  
  params$restart <- c()
  
  if ("LAI" %in% nc_var.names) {
    forecast[[length(forecast) + 1]] <- ens$LAI[last] ## m2 m-2 
    names(forecast[[length(forecast)]]) <- c("LAI")
  }
  
  if ("fast_soil_pool_carbon_content" %in% nc_var.names) {
  ...
  if ("slow_soil_pool_carbon_content" %in% nc_var.names) {
  ...
  if ("nonelongating_generative_tiller" %in% nc_var.names) {
  ...
  if ("elongating_generative_tiller" %in% nc_var.names) {
  ...
  if ("nonelongating_vegetative_tiller" %in% nc_var.names) {
